...
import { getCsvValue, getGroupedBarChartData, getPieChartSlices, getTimeSeriesData, parseCsv } from "../utils/csv";
...
export default function ReportView({ csvContents, report }) {
  ...
  const groupedBarChartData = report.visualization?.type === "grouped-bar" ? getGroupedBarChartData(csvContent, report.visualization.categoryColumn, report.visualization.seriesColumn, report.visualization.valueColumn, report.visualization.categoryLabels, report.visualization.seriesLabels) : null;
  const timeSeriesData = report.visualization?.type === "time-series" ? getTimeSeriesData(csvContent, report.visualization.dateColumn, report.visualization.seriesColumn, report.visualization.valueColumn, report.visualization.seriesLabels) : null;
  ...
  return (
    <>
      ...
      {pieChartSlices ? <PieChart ariaLabel={report.visualization.ariaLabel} slices={pieChartSlices} summaryItems={summaryItems} totalLabel={report.visualization.totalLabel} valueLabel={report.visualization.valueLabel} /> : groupedBarChartData ? <GroupedBarChart ariaLabel={report.visualization.ariaLabel} categories={groupedBarChartData.categories} series={groupedBarChartData.series} valueLabel={report.visualization.valueLabel} /> : timeSeriesData ? <TimeSeriesChart ariaLabel={report.visualization.ariaLabel} records={timeSeriesData.records} series={timeSeriesData.series} valueLabel={report.visualization.valueLabel} /> : <DataTable columns={columns ?? []} rows={rows} />}
    </>
  );
}
