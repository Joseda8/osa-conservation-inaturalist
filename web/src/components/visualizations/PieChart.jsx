      ...
      <div className="pie-chart" role="img" aria-label={ariaLabel}>
        <svg viewBox="0 0 100 100">
          {visibleSlices.map((slice, sliceIndex) => {
            ...
            startAngle = endAngle;
            return <path aria-label={`${slice.label}: ${slice.count.toLocaleString()} ${valueLabel}`} className={sliceIndex === hoveredSliceIndex ? "pie-slice active" : "pie-slice"} d={path} fill={slice.color} key={slice.id} onBlur={() => setHoveredSliceIndex(null)} onFocus={() => setHoveredSliceIndex(sliceIndex)} onMouseEnter={() => setHoveredSliceIndex(sliceIndex)} onMouseLeave={() => setHoveredSliceIndex(null)} tabIndex="0" />;
          })}
        </svg>
      </div>
