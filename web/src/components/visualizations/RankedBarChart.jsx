...
export default function RankedBarChart({ ariaLabel, groups, rankLimitOptions = [], valueLabel }) {
  ...
  const visibleItems = activeGroup?.items.filter((item) => activeRankLimit === null || item.rank <= activeRankLimit) ?? [];
  const maximumValue = Math.max(...visibleItems.map((item) => item.value), 0);
  ...
  return (
    <section aria-label={ariaLabel} className="ranked-bar-chart">
      {groups.length > 1 && <div aria-label="Choose region" className="ranked-bar-chart-selector">
        <span>Show:</span>
        {groups.map((group) => (
      ...
      {rankLimitOptions.length > 1 && <div aria-label="Choose ranking size" className="ranked-bar-chart-selector">
        <span>Show:</span>
        {rankLimitOptions.map((rankLimitOption) => <button aria-pressed={rankLimitOption.value === activeRankLimit} className={rankLimitOption.value === activeRankLimit ? "ranked-bar-chart-selector-item active" : "ranked-bar-chart-selector-item"} key={rankLimitOption.label} onClick={() => { setActiveRankLimit(rankLimitOption.value); setActiveItemId(null); }} style={{ "--ranked-bar-color": activeGroup.color }} type="button">{rankLimitOption.label}</button>)}
      </div>}
