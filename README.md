...
### Enrich taxonomy
Completes the `taxa` table with metadata for every ID referenced by stored taxon lineages. It also retrieves each taxon's conservation statuses directly from iNaturalist once, separately from raw observation downloads. Missing mode requests absent lineage nodes and taxa that have not yet received this status enrichment. Full mode refreshes all stored taxa and then completes any newly discovered lineage nodes.
...
The step first writes ABS-vs-OBS CSV reports for total observations, observations by day, duplicate observation IDs, quality grades, species reported over time, and observer metrics to `data/processed-data/YYYYMMDD/`. Re-running it on the same date replaces that date's files. It mirrors this structure in Google Drive as `processed-data/YYYYMMDD/`, updating matching files in that dated folder. GitHub Pages always reads the latest dated Drive folder. If authorization has not been completed, it skips the upload and leaves the generated local reports available; it never opens a browser. Run `--pipeline auth-with-google` first. Set `GOOGLE_DRIVE_UPLOAD_FOLDER_ID` in the ignored `.env` file to the ID of OSA's `processed-data` folder. Never commit either OAuth JSON file. Set `GOOGLE_DRIVE_OAUTH_CLIENT_JSON_PATH` or `GOOGLE_DRIVE_OAUTH_TOKEN_PATH` to use different local paths.

Authorize the Google user before uploading reports:

```bash
PYTHONPATH=src python3 src/main.py --pipeline auth-with-google
```

This opens a browser and saves the selected user's refresh token at `.secrets/google-oauth-token.json` without generating reports or modifying Drive.
